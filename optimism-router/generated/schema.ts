...
export class LogAnySwapOut1 extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save LogAnySwapOut1 entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type LogAnySwapOut1 must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("LogAnySwapOut1", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): LogAnySwapOut1 | null {
    return changetype<LogAnySwapOut1 | null>(
      store.get("LogAnySwapOut1", id.toHexString())
    );
  }
